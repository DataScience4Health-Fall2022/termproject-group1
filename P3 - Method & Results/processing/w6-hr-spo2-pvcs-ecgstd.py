...
for pid in training_patients:
    ...
    for i in range(len(hr)):
        ecgstd = np.std(ecg[i])
        if (np.isnan(hr[i]) or np.isnan(spO2[i]) or np.isnan(pvcs[i]) or np.isnan(ecgstd)):
            continue
        X.append([hr[i], spO2[i], pvcs[i], ecgstd])
        y.append(labels[i])
...
for pid in testing_patients:
    ...
    for i in range(len(hr)):
        ...
        if (np.isnan(hr[i]) or np.isnan(spO2[i]) or np.isnan(pvcs[i]) or np.isnan(ecgstd)):
            continue
        X_t.append([hr[i], spO2[i], pvcs[i], ecgstd])
        y_t.append(labels[i])

X_t = StandardScaler().fit_transform(X_t)

print("w6 hr, spo2, pvcs, ecgstd")
