...
for pid in training_patients:
    ...
    for i in range(len(hr)):
        ecgstd = np.std(ecg[i])
        psgflowstd = np.std(psgflow[i])

        if (np.isnan(hr[i]) or np.isnan(spO2[i]) or np.isnan(pvcs[i]) or np.isnan(ecgstd) or np.isnan(psgflowstd)):
            continue
        X.append([hr[i], spO2[i], pvcs[i], ecgstd, psgflowstd])
        y.append(labels[i])
...
for pid in testing_patients:
    ...
    for i in range(len(hr)):
        ...
        if (np.isnan(hr[i]) or np.isnan(spO2[i]) or np.isnan(pvcs[i]) or np.isnan(ecgstd) or np.isnan(psgflowstd)):
            continue
        X_t.append([hr[i], spO2[i], pvcs[i], ecgstd, psgflowstd])
        y_t.append(labels[i])

X_t = StandardScaler().fit_transform(X_t)

print("w6 hr, spo2, pvcs, ecgstd, psgflowstd")
